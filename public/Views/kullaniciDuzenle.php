...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['bilgileriGuncelle'])) {
		...
		if ($sifre === $sifre2) {
			...
			$sonuc = $kullaniciDuzenleController->sifreGuncelle($id, $hashedPassword);
			if ($sonuc) {
				echo "<script>alert('Şifre başarıyla güncellendi.'); window.location.href = 'kullanici.php';</script>";
			} else {
